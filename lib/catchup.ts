...
export async function runDailyHealthCheck(): Promise<boolean> {
  ...
  try {
    ...
    await sendDailyHealthReport({
      health:             { db: dbOk, ai: aiOk, aiProvider, grok: grokHealth.ok, grokDetail: grokHealth.detail },
      generatedPosts,
    ...
    console.log(`[DailyHealth] Report sent — published24h: ${publishedCount24h}, failed24h: ${failedPosts24h.length}, rateLimits: ${rateLimitEvents.length}, errors: ${systemErrors.length}, ytPosts24h: ${youtubePosts24h.length}`);
    return true;
  } catch (err: any) {
