...
- **🎙️ AI voiceover + auto-translated captions = a global audience.** Opt in and every Short is narrated by a natural AI voice (six to choose from) over auto-ducked music — with **perfectly synced per-card narration** (each card is spoken while it's on screen, and the Short grows to fit longer cards). With captions left to YouTube, your Shorts get **auto-translated into every viewer's language** for free. Want them baked in? Flip on TikTok-style word-by-word burned captions.
...
> 📚 **Full setup, API-key sourcing, environment variables, deployment, and troubleshooting:** see the **[full README](README.md)**.
